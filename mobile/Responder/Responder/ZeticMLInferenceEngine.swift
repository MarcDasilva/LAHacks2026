...
final class QwenOmniTranscriptionEngine: AudioTranscriptionEngine {
    ...
    private func loadAudioEncoderWithRetry(candidateKeys: [String]) throws -> ZeticMLangeModel {
        ...
        for key in keys {
            for attempt in 1...3 {
                do {
                    print("[Responder][STT] Loading encoder model=\(settings.qwenAudioEncoderModelName) attempt=\(attempt) keyPrefix=\(keyPrefix(key))")
                    return try ZeticMLangeModel(
                        personalKey: key,
                        name: settings.qwenAudioEncoderModelName,
        ...
        throw makeSTTStageError(
            ...
            modelName: settings.qwenAudioEncoderModelName,
            underlying: lastError ?? NSError(
            domain: "Responder.STT",
            code: -10,
    ...
    private func loadDecoderWithRetry(candidateKeys: [String]) throws -> ZeticMLangeLLMModel {
        ...
        for key in keys {
            for attempt in 1...3 {
                do {
                    print("[Responder][STT] Loading decoder model=\(settings.qwenDecoderModelName) attempt=\(attempt) keyPrefix=\(keyPrefix(key))")
        ...
        throw makeSTTStageError(
            stage: "load_decoder",
            modelName: settings.qwenDecoderModelName,
            underlying: lastError ?? NSError(
            domain: "Responder.STT",
            code: -11,
